...
import { GET_DIVISION_BY_PATH } from '@/lib/queries'
import { DrupalDivision } from '@/lib/types'
...
interface DivisionByPathData {
  route: {
    entity: DrupalDivision
  } | null
}

async function getDivision(path: string): Promise<DrupalDivision | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<DivisionByPathData>({
      query: GET_DIVISION_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/divisions/${slug.join('/')}`
  const item = await getDivision(path)

  if (!item) {
    return { title: 'Division Not Found | Court' }
  }

  return {
    title: `${item.title} | Court`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).chiefJudge && (
                    <div>
                      <dt className="text-sm text-gray-500">Chief Judge</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).chiefJudge}</dd>
                    </div>
                  )}
