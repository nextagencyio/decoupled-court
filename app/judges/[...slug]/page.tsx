...
import { GET_JUDGE_BY_PATH } from '@/lib/queries'
import { DrupalJudge } from '@/lib/types'
...
async function getJudge(path: string): Promise<DrupalJudge | null> {
  try {
    ...
    const { data } = await apolloClient.query<JudgeByPathData>({
      query: GET_JUDGE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/judges/${slug.join('/')}`
  const item = await getJudge(path)

  if (!item) {
    return { title: 'Judge Not Found | Court' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).chambers && (
                    <div>
                      <dt className="text-sm text-gray-500">Chambers</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).chambers}</dd>
                    </div>
                  )}
                  {(item as any).phone && (
                    <div>
