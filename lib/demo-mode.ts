...
import judgesData from '@/data/mock/judges.json'
import divisionsData from '@/data/mock/divisions.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetJudges') || query.includes('nodeJudges')) {
      return loadMockData('judges.json')
    }

    if (query.includes('GetDivisions') || query.includes('nodeDivisions')) {
      return loadMockData('divisions.json')
    }
